...
for m in $MODELS; do
    ...
    trt_cache_pre
    python -u ${BASE_DIR}/main.py -m ${MODEL_DIR}/${m} -s model -b ${bs} -o fp16,use_ncu -D 32,16 -v -f ${WORK_DIR}/${m}-fp16.json 2>&1 | tee ${WORK_DIR}/${m}-fp16.log
    trt_cache_post
...
bs=1
trt_cache_pre
python -u ${BASE_DIR}/main.py -m ${MODEL_DIR}/${m} -s model -b ${bs} -o use_ncu,fp16 -D 32,16 -v --inputs-shape-override='{"sample": [null, 4, 128, 128], "encoder_hidden_states": [null, 128, null]}' -f ${WORK_DIR}/${m}-fp16.json 2>&1 | tee ${WORK_DIR}/${m}-fp16.log
trt_cache_post
...
bs=128
trt_cache_pre
python -u ${BASE_DIR}/main.py -m ${MODEL_DIR}/${m} -s model -b ${bs} -o use_ncu,fp16 -D 32,16 -v --inputs-shape-override='{"input_ids": [null, 512], "attention_mask": [null, 512]}' -f ${WORK_DIR}/${m}-fp16.json 2>&1 | tee ${WORK_DIR}/${m}-fp16.log
trt_cache_post
...
python ${BASE_DIR}/test/helper_fig2_single.py 1 ${WORK_DIR}/e2e.csv ${RESULT_DIR}/figure2_subplot
