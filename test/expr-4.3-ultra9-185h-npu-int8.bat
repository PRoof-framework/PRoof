...
REM normal models
set MODELS=efficientnet_b0.onnx efficientnet_b4.onnx efficientnetv2_rw_s.onnx efficientnetv2_rw_t.onnx mobilenetv2_050.onnx mobilenetv2_100.onnx resnet34.onnx resnet50.onnx

set bs=1
for %%m in (%MODELS%) do (
    echo %%m
    python -u %BASE_DIR%\main.py -B openvino -o device=NPU,int8 -m %MODEL_DIR%\%%m -s model -b %bs% -D 32,8 -v -f %WORK_DIR%\%%m.json 2>&1 > %WORK_DIR%\%%m.log
)
