...
for m in $MODELS; do
    ...
    trt_cache_pre
    python -u ${BASE_DIR}/main.py -m ${MODEL_DIR}/${m} -s model -b ${bs} -o int8 -D 32,8 -v -f ${WORK_DIR}/${m}-int8.json 2>&1 | tee ${WORK_DIR}/${m}-int8.log
    trt_cache_post
...
python ${BASE_DIR}/test/helper_fig2_single.py 6 ${WORK_DIR}/e2e.csv ${RESULT_DIR}/figure2_subplot
